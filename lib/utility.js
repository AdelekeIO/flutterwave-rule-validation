...
const conditionData = config.get("condition");
const inputRequiredFields = config.get("inputRequiredFields"); //
const ruleData = config.get("rule"); //
...
exports.conditionValidation = async (payload) => {
  try {
    ...
    let result = "";
    let { values: conditionsDefinition } = conditionData;
    let validation = {
      error: false,
      field: "[name of field]",
...
module.exports.validate = async (payload) => {
  try {
    ...
    let resp = await hasFields(inputRequiredFields, payload);
